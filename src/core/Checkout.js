...
const Checkout = ({ cart, emptyCart }) => {
  ...
  useEffect(() => {
    const getToken = (id) => {
      getBraintreeClientToken(id).then((data) => {
        if (data.error) {
      ...
      {paymentData.clientToken !== null && cart.length > 0 ? (
        <div>
          ...
          <DropIn
            ...
            onInstance={(instance) => {
              return (paymentData.instance = instance);
            }}
          />
  ...
  const showError = (error) => (
    ...
    >
      {error === "No payment method is available."
        ? "You didn't choose a form of payment yet. Please select a form of payment below:"
        : error}
    </div>
  );
  ...
  const showSuccess = () => (
    <div
      ...
      style={{ display: paymentData.success ? "" : "none" }}
    >
